...
if(isset($_GET['search_prop_id']) and  $_GET['search_prop_id']!= "")
{
    $args['p'] = intval($_GET['search_prop_id']);
}
else
{
    ...
    if ( isset($_GET['search_phone1']) and  $_GET['search_phone1']!= "" OR isset($_GET['search_landlord_name']) and  $_GET['search_landlord_name']!= "")
    {
        if ( isset($_GET['search_phone1']) and  $_GET['search_phone1']!= "")
        {
            $meta_query[] = array(
                'key' => 'phone1',
                'value' => trim($_GET['search_phone1']),
                'compare' => 'like'
            );
        }

        if ( isset($_GET['search_landlord_name']) and  $_GET['search_landlord_name']!= "")
        {
            $meta_query[] = array(
                'key' => 'landlord_name',
                'value' => trim($_GET['search_landlord_name']),
                'compare' => 'like'
            );
        }
    ...
    else
    {
        ...
        if ( isset($_GET['search_city_select']) and  $_GET['search_city_select']!= "")
        {
            $meta_query[] = array(
                'key' => 'city_name',
                'value' => trim($_GET['search_city_select']),
                'compare' => 'like'
            );
        }

	    if ( isset($_GET['search_region_select']) and  $_GET['search_region_select']!= "" and $_GET['search_region_select']!= 0)
	    {
	        $region_district=Utility::get_region_district($_GET['search_region_select']);
		    $meta_query[] = array(
			    'key' => 'district_name',
	    ...
	    {
		    if ( isset($_GET['search_district_select']) and  $_GET['search_district_select']!= "")
		    {
			    $meta_query[] = array(
				    'key' => 'district_name',
				    'value' => explode('|',$_GET['search_district_select']),
				    'compare' => 'IN'
			    );
		    }
	    }
        ...
        if ( isset($_GET['search_deal_type_select']) and  $_GET['search_deal_type_select']!= "")
        {
            $status="";
            switch ($_GET['search_deal_type_select'])
            {
                case 'رهن واجاره':
                    $status="رهن و اجاره";
                    if(isset($_GET['search_mortgage_select']) and  $_GET['search_mortgage_select']!= "")
                    {
                        $price= explode('|',$_GET['search_mortgage_select']);
                        sort($price);
                        $price = array_map( function($val) { return $val * 1000000; }, $price);
                        if (count($price) == 1)
                        {
                            $meta_query[] = array(
                                'key' => 'mortgage',
                    ...
                    if(isset($_GET['search_rent_select']) and  $_GET['search_rent_select']!= "")
                    {
                        $rent= explode('|',$_GET['search_rent_select']);
                        sort($rent);
                        $rent = array_map( function($val) { return $val * 1000; }, $rent);
                        if (count($rent) == 1)
                        {
                            $meta_query[] = array(
                                'key' => 'rent',
                ...
                case 'خرید':
                    $status="فروش";
                    if(isset($_GET['search_price_select']) and  $_GET['search_price_select']!= "")
                    {
                        $price= explode('|',$_GET['search_price_select']);
                        sort($price);
                        $price = array_map( function($val) { return $val * 1000000; }, $price);
                        if (count($price) == 1)
                        {
                    ...
                    if(isset($_GET['search_deed_select']) and  $_GET['search_deed_select']!= "")
                    {
                        $deed= explode('|',$_GET['search_deed_select']);
                        $meta_query[] = array(
                            'key' => 'deed',
                            'value' => $deed,
                            'compare' => 'IN'
        ...
        if ( isset($_GET['search_prop_type_select']) and  $_GET['search_prop_type_select']!= "")
        {
            $meta_query[] = array(
                'key' => 'type',
                'value' => trim($_GET['search_prop_type_select']),
                'compare' => 'like'
            );
        }
        ...
        if ( isset($_GET['search_deed_select']) and  $_GET['search_deed_select']!= "")
        {
            $meta_query[] = array(
                'key' => 'deed',
                'value' => explode('|',$_GET['search_deed_select']),
                'compare' => 'IN'
            );
        }


        if ( isset($_GET['search_bedroom_select']) and  $_GET['search_bedroom_select']!= "")
        {
            $bedroom= explode('|',$_GET['search_bedroom_select']);
            sort($bedroom);
            if (count($bedroom) == 1)
            {
                $meta_query[] = array(
        ...
        if ( isset($_GET['search_facilities_select']) and  $_GET['search_facilities_select']!= "")
        {
            $facilities=explode('|',$_GET['search_facilities_select']);
        ...
        if ( isset($_GET['search_floor_select']) and  $_GET['search_floor_select']!= "")
        {
            $floor= explode('|',$_GET['search_floor_select']);
            sort($floor);
            if (count($floor) == 1)
            {
                $meta_query[] = array(
        ...
        if ( isset($_GET['search_infrastructure_select']) and  $_GET['search_infrastructure_select']!= "")
        {
            $infrastructure= explode('|',$_GET['search_infrastructure_select']);
            sort($infrastructure);
            if (count($infrastructure) == 1)
            {
                $meta_query[] = array(
        ...
        if ( isset($_GET['search_age_select']) and  $_GET['search_age_select']!= "")
        {
            $age= explode('|',$_GET['search_age_select']);
            sort($infrastructure);
            if (count($infrastructure) == 1)
            {
                $meta_query[] = array(
        ...
        if ( isset($_GET['search_land_area_select']) and  $_GET['search_land_area_select']!= "")
        {
            $land_area= explode('|',$_GET['search_land_area_select']);
            sort($land_area);
            if (count($land_area) == 1)
            {
                $meta_query[] = array(
                    'key' => 'land_area',
                    'value'   =>$land_area[0] ,
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="row align-items-center pb-1 pt-3 mb-3" >
                                    ...
                                    <form class="form-inline well" id="aral-home-search-form" method="get" action="<?php echo home_url('aral-all-prop')?>">
                                        ...
                                        if (is_user_logged_in() and Utility::is_user_exp(get_current_user_id()))
                                        {
                                            ...
                                            <div class="form-group mx-sm-3 mb-2" id="search_landlord_name_div">
                                                <input type="text" class="form-control form-rounded text-center" placeholder="نام و نام خانوادگی مالک" id="search_landlord_name" name="search_landlord_name" style="border-color: rgb(189 189 189);font-family: 'iransans-light' !important;" <?php echo (isset($_GET['search_landlord_name']) and $_GET['search_landlord_name'] !='')?'value="'.$_GET['search_landlord_name'].'"':'';?>>
                                            </div>
                                            <div class="form-group mx-sm-3 mb-2" id="search_phone1_div">
                                                <input type="text" class="form-control form-rounded text-center" placeholder="شماره تلفن مالک" id="search_phone1" name="search_phone1" style="border-color: rgb(189 189 189);font-family: 'iransans-light' !important;" <?php echo (isset($_GET['search_phone1']) and $_GET['search_phone1'] !='')?'value="'.$_GET['search_phone1'].'"':'';?>>
                                            </div>
                                            <?php
                                        }
                                        ?>
                                ...
                                <div class="col-md-4">
                                    <img src="<?php echo $dash_assets_url.'assets/media/svg/undraw_onboarding_o8mv.svg'?>" alt="..." class="img-fluid" usemap="#image-map">
                                    <map name="image-map" id="image-map">
                                        <area target="" alt="آپارتمان" title="آپارتمان" href="" coords="338,343,743,204" shape="rect" id="img_aparteman" data-toggle="tooltip" data-placement="top" title="لیست شرکت کنندگان در مناقصه">
                                        <area target="" alt="ویلایی" title="ویلایی" href="" coords="339,392,740,539" shape="rect" id="img_villa" data-toggle="tooltip" data-placement="top" title="لیست شرکت کنندگان در مناقصه">
                                        <area target="" alt="رهن و اجاره" title="رهن و اجاره" href="" coords="338,573,740,720" shape="rect" id="img_rent" data-toggle="tooltip" data-placement="top" title="لیست شرکت کنندگان در مناقصه">
                                    </map>
                                </div>
                              </div>
                     </div>
                        ...
                        <?php
                            if(isset($_GET['search_city_select'],$_GET['search_deal_type_select'],$_GET['search_prop_type_select']))
                            {
                                ...
                                <div class="mr-3 d-sm-inline d-none">
                                    <?php
                                    $deal_prop_city="";
                                    if(isset($_GET['search_deal_type_select']))
                                    {
                                        $deal_prop_city.=$_GET['search_deal_type_select'];
                                    }

                                    if(isset($_GET['search_prop_type_select']))
                                    {
                                        $deal_prop_city.=' | '.$_GET['search_prop_type_select'];
                                    }

                                    if(isset($_GET['search_city_select']))
                                    {
                                        $deal_prop_city.=' | '.$_GET['search_city_select'];
                                    }
                                    echo '<button type="button" class="btn btn-warning">
                                            <i class="fa fa-building-o mr-2"></i> '. $deal_prop_city.'
                                      </button>';
                                    ...
                                    if(isset($_GET['search_price_select']) and $_GET['search_price_select'] !="" )
                                    {
                                        ...
                                        if (count($price) == 1)
                                        {
                                            if ($price[0] >= 1000)
                                            {
                                                $price_title.='قیمت تا '.($price[0]/1000).' میلیارد';
                                            }
                                            else
                                            {
                                                $price_title.='قیمت تا '.$price[0].' میلیون';
                                            }
                                            echo '<button type="button" class="btn btn-yahoo ml-3">
                                                        <i class="fa fa-usd fa-lg mr-2"></i> '. $price_title.'
                                                  </button>';
                                        }
                                        else
                                        {
                                            if ($price[0] >= 1000)
                                            {
                                                $price_title.='قیمت از '.($price[0]/1000).' میلیارد';
                                            }
                                            else
                                            {
                                                $price_title.='قیمت از '.$price[0].' میلیون';
                                            }

                                            if ($price[1] >= 1000)
                                            {
                                                $price_title.=' تا '.($price[1]/1000).' میلیارد';
                                            }
                                            else
                                            {
                                                $price_title.=' تا '.$price[1].' میلیون';
                                            }
                                            echo '<button type="button" class="btn btn-yahoo ml-3">
                                                        <i class="fa fa-usd fa-lg mr-2"></i> '. $price_title.'
                                                  </button>';
                                    ...
                                    if(isset($_GET['search_mortgage_select']) and $_GET['search_mortgage_select'] !="")
                                    {
                                        ...
                                        if (count($price) == 1)
                                        {
                                            if ($price[0] >= 1000)
                                            {
                                                $price_title.='رهن تا '.($price[0]/1000).' میلیارد';
                                            }
                                            else
                                            {
                                                $price_title.='رهن تا '.$price[0].' میلیون';
                                            }
                                            echo '<button type="button" class="btn btn-yahoo ml-3">
                                                        <i class="fa fa-usd fa-lg mr-2"></i> '. $price_title.'
                                                  </button>';
                                        ...
                                        else
                                        {
                                            if ($price[0] >= 1000)
                                            {
                                                $price_title.='رهن از '.($price[0]/1000).' میلیارد';
                                            }
                                            else
                                            {
                                                $price_title.='رهن از '.$price[0].' میلیون';
                                            }
                                    ...
                                    if(isset($_GET['search_rent_select']) and $_GET['search_rent_select'] !="")
                                    {
                                        $price= explode('|',$_GET['search_rent_select']);
                                        sort($price);
                                        $price_title="";
                                        if (count($price) == 1)
                                        {
                                            if ($price[0] >= 1000)
                                            {
                                                $price_title.='اجاره تا '.($price[0]/1000).' میلیارد';
                                            }
                                            else
                                            {
                                                $price_title.='اجاره تا '.$price[0].' میلیون';
                                            }
                                            echo '<button type="button" class="btn btn-yahoo ml-3">
                                                        <i class="fa fa-usd fa-lg mr-2"></i> '. $price_title.'
                                                  </button>';
                                        ...
                                        else
                                        {
                                            if ($price[0] >= 1000)
                                            {
                                                $price_title.='اجاره از '.($price[0]/1000).' میلیون';
                                            }
                                            else
                                            {
                                                $price_title.='اجاره از '.$price[0].' هزار';
                                            }

                                            if ($price[1] >= 1000)
                                            {
                                                $price_title.=' تا '.($price[1]/1000).' میلیون';
                                            }
                                            else
                                            {
                                                $price_title.=' تا '.$price[1].' هزار';
                                            }
                                            echo '<button type="button" class="btn btn-yahoo ml-3">
                                                        <i class="fa fa-usd fa-lg mr-2"></i> '. $price_title.'
                                                  </button>';
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-body">
                                    <div class="row align-items-center pb-1 pt-3 mb-3" >
                                        <div class="col-md-7">
                                          <h2>متاسفانه ملــکی با مشخــصات مدنظــر جنابعالی یافت نشد!</h2>
                                        </div>
                                        <div class="col-md-5">
