...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        npr_price.on("change", function(item, state)
        {
            ...
            switch (selected_value.length)
            {
                case 0:
                    ...
                    break;
                case 1:
                    npr_price.updateLabel('از' + ' '+selected_value[0]+' '+'میلیون');
                    break;
                case 2:
        ...
        npr_infrastructure_area.on("change", function(item, state)
        {
            ...
            switch (selected_value.length)
            {
                case 0:
                    ...
                    break;
                case 1:
                    npr_infrastructure_area.updateLabel('از' + ' '+selected_value[0]+' '+'متر');
                    break;
                case 2:
        ...
        npr_mortgage.on("change", function(item, state)
        {
            ...
            switch (selected_value.length)
            {
                case 0:
                    ...
                    break;
                case 1:
                    npr_mortgage.updateLabel('از' + ' '+selected_value[0]+' '+'میلیون');
                    break;
                case 2:
        ...
        npr_rent.on("change", function(item, state)
        {
            ...
            switch (selected_value.length)
            {
                ...
                case 1:
                    ...
                    {
                        npr_rent.updateLabel('از' + ' '+selected_value[0]+' '+'هزار');
                    }
                    else
                    {
        ...
        function fetch_user_prop_requests()
        {
            ...
            $('#user_prop_request_tbl').DataTable({
                ...
                ajax: {
                    url: '<?php echo admin_url("admin-ajax.php");?>',
                    data: {
                        action: 'admin_get_all_prop_request',
                        user_id: '<?php echo get_current_user_id()?>',
                    },
                    type: 'post',
                },
        ...
        $(document.body).on('click','#npr_search',function () {
            $(this).closest('tr').addClass(' loadingbar gray').prop('disabled',true );
            fetch_matched_prop($(this).data('request_id'));
            $(this).closest('tr').removeClass(' loadingbar gray').prop('disabled',true );
        });


       function fetch_matched_prop(request_id)
        {
            $("#matched_prop_div").removeClass('d-none').slideDown("slow");
            $("#matched_prop_tbl").addClass('loadingbar gray');
            $('#matched_prop_tbl').DataTable().clear().destroy();
            $('#matched_prop_tbl').DataTable({
                language: {
                    processing:     "در حال پردازش",
                    searchPlaceholder: "جستجو ...",
                    lengthMenu:    " _MENU_",
                    emptyTable:     "ملکی مشابه با این درخواست یافت نشد!",
                ...
                "fnDrawCallback" : function() {
                    traverse(document.body);
                    $("#matched_prop_tbl").removeClass('loadingbar gray');
                },
                "rowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
        ...
        $(document.body).on('click','#exclude_prop',function () {
            ...
            $.confirm({
                ...
                content: '' +
                    ...
                    '<textarea  placeholder="دلیل حذف این ملک از لیست پیشنهادی" class="prop_exclude_description form-control" required></textarea>' +
                    '</div>' +
                    '</form>',
                buttons: {
                    submit: {
                        ...
                        btnClass: 'btn-green',
                        action: function(){
                            var description = this.$content.find('.prop_exclude_description').val();
                            if(!description){
                                $.alert('توضیحات الزامی است!');
                                return false;
                            }
                            $.post(
                                ...
                                function (response)
                                {
                                    ...
                                    else if(response.status == 'not_update')
                                    {
                                        toastr.options = {
                                            timeOut: 4000,
                                            positionClass: "toast-top-left",
        ...
        $(document.body).on('click','#npr_finish',function () {
            var request_id=$(this).data('request_id');
            $.confirm({
                boxWidth: '30%',
                useBootstrap: false,
                ...
                content: '' +
                    '<form action="" class="formName">' +
                    '<div class="form-group" style="margin-right: 10px;">' +
                    '<textarea  placeholder="توضیحات" class="request_description form-control" required></textarea>' +
                    '</div>' +
                    '</form>',
                buttons: {
                    submit: {
                        text: 'تکمیل این درخواست',
                        btnClass: 'btn-green',
                        action: function(){
                            var description = this.$content.find('.request_description').val();
                            if(!description){
                                $.alert('توضیحات الزامی است!');
                                return false;
                            }
                            $.post(
                                ...
                                function (response)
                                {
                                    if (response.status == 'update')
                                    {
                                        ...
                                        $('#compelted_requests').html(result.complete_request+' '+'درخواست');
                                        $('#pending_requests').html(result.pending_request+' '+'درخواست');
                                        toastr.success(response.msg);
                                    }
                                    else
                                    {
        ...
        $(document.body).on('click','#npr_new_added_prop',function () {
            $(this).closest('tr').addClass(' loadingbar gray').prop('disabled',true );
            fetch_new_matched_prop($(this).data('request_id'));
            $(this).closest('tr').removeClass(' loadingbar gray').prop('disabled',true );
            $('button#check_all_new_result').attr('data-request_id', $(this).data('request_id'));
        });


        function fetch_new_matched_prop(request_id)
        {
            $("#new_matched_prop_div").removeClass('d-none').slideDown("slow");
            $("#new_matched_prop_tbl").addClass('loadingbar gray');
            $('#new_matched_prop_tbl').DataTable().clear().destroy();

            $('#new_matched_prop_tbl').DataTable({
                language: {
                    processing:     "در حال پردازش",
                    searchPlaceholder: "جستجو ...",
                    lengthMenu:    " _MENU_",
                ...
                "fnDrawCallback" : function() {
                    traverse(document.body);
                    $("#new_matched_prop_tbl").removeClass('loadingbar gray');
                    $('#user_prop_request_tbl').DataTable().clear().destroy();
                    fetch_user_prop_requests();
                },
                "rowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
        ...
        $(document.body).on('click','#new_result_exclude_prop',function () {
            var prop_id=$(this).data('prop_id');
            var request_id=$(this).data('request_id');
            $.confirm({
                boxWidth: '30%',
                    ...
                    submit: {
                        ...
                        action: function(){
                            ...
                            $.post(
                                '<?php echo admin_url("admin-ajax.php");?>',
                                {
                                    action  : 'new_result_exclude_prop',
                                    request_id : request_id,
                                    prop_id : prop_id,
                                    user_id : '<?php echo get_current_user_id()?>',
                                    description :description,
                                ...
                                function (response)
                                {
                                    ...
                                    {
                                        $('#new_matched_prop_tbl').DataTable().clear().destroy();
                                        fetch_new_matched_prop(request_id);
                                        toastr.options = {
                                            timeOut:4000,
                                            positionClass: "toast-top-left",
                                            progressBar: true,
        ...
        $(document.body).on('click','#check_all_new_result',function () {
            var request_id=$(this).data('request_id');
            $.confirm({
                boxWidth: '30%',
                useBootstrap: false,
                ...
                rtl: true,
                closeIcon: true,
                content: 'لیست پیشنهادی جدید را بررسی کردید؟',
                buttons: {
                    submit: {
                        text: 'بله بررسی کردم',
                        btnClass: 'btn-green',
                        action: function(){
                            $.post(
                                '<?php echo admin_url("admin-ajax.php");?>',
                                {
                                    action  : 'complete_all_new_result',
                                    request_id : request_id,
                                    user_id : '<?php echo get_current_user_id()?>',
                                },
                                function (response)
                                {
                                    if (response.status == 'update')
                                    {
                                        $('#user_prop_request_tbl').DataTable().clear().destroy();
                                        fetch_user_prop_requests();
                                        $("#new_matched_prop_div").addClass('d-none');
                                        toastr.options = {
                                            timeOut:4000,
                                            positionClass: "toast-top-left",
                                            progressBar: true,
        ...
        $(document.body).on('click','#show_excluded_prop',function () {
            var temp=$(this);
            $(this).closest('tr').addClass(' loadingbar gray').prop('disabled',true );
            $.ajax({
                type: "POST",
                url: '<?php echo admin_url('admin-ajax.php');?>',
                data :{
                    action : 'admin_show_excluded_prop',
                    r_id : $(this).data('request_id'),
                    user_id:<?php echo get_current_user_id()?>,
                },
                success:function (result) {
                    $('div #show_excluded_prop_modal_tbl').html('');
                    $('div #show_excluded_prop_modal_tbl').append(result);
                    temp.closest('tr').removeClass(' loadingbar gray');
                    $('#show_excluded_prop_modal').modal('toggle');
                }
            });
        });

        $(document.body).on('click','#npr_change_exp',function () {
            var temp=$(this);
            var request_id=temp.data('request_id');
            $(this).closest('tr').addClass(' loadingbar gray').prop('disabled',true );
            $.ajax({
                type: "POST",
                url: '<?php echo admin_url('admin-ajax.php');?>',
                data :{
                    action : 'admin_get_all_exp',
                    r_id : $(this).data('request_id'),
                    user_id:<?php echo get_current_user_id()?>,
                },
                success:function (result) {
                    ...
                    $.confirm({
                        boxWidth: '30%',
                        useBootstrap: false,
                        title: 'ویرایش کارشناس این درخواست',
                        rtl: true,
                        closeIcon: true,
                        content: '' +
                            '<form action="" class="formName">' +
                        ...
                        buttons: {
                            submit: {
                                ...
                                btnClass: 'btn-green',
                                action: function(){
                                    var exp_id = this.$content.find('.exp_list').val();

                                    $.post(
                                        '<?php echo admin_url("admin-ajax.php");?>',
                                        {
                                            action  : 'admin_edit_exp',
                                            request_id : request_id,
                                            exp_id : exp_id,
                                            user_id:<?php echo get_current_user_id();?>
                                        },
